export const Lang = {
  en: {
    Login: "Login",
    phone: "Phone Number",
    googleAuth: "Login With Google",
    loginTitle: "Enjoy the convenience of using your membership",
    loginSubTitle: "Launching new features",
    loginSubTitle2:
      "Get discounted prices at top medical centers without paying the full price.",
    home: "Home",
    network: "Medical Network",
    contactUs: "Contact Us",
    hero: "Celebrity membership your beauty deserves it",
    heroSub: "Your gateway to exclusive membership in health and beauty",
    heroBullet1:
      "Exclusive discounts up to 80% ✦ Trusted, growing clinic network",
    heroBullet2: "Full digital membership — 299 SAR / year",
    heroBullet3: "Active instantly ✓ No hidden terms",
    heroBtn: "Get your membership",
    heroTrust: "✓ 100% digital ✓ Active instantly ✓ No hidden terms",
    howSectionTitle: "How does Glow Card work?",
    howStep1:
      "Choose your membership — Classic, Premium, Couples, Family, or Business — or pick what fits your life",
    howStep2:
      "Activate your membership — Pay 299 SAR, your membership activates instantly and you start saving right away",
    howStep3:
      "Choose your clinic — Browse our growing network of trusted beauty and wellness clinics in your city",
    howStep4:
      "Show your digital membership at the clinic — Save and get your discount on the spot, no papers or coupons",
    whySectionTitle: "Why Glow Card?",
    why1Title: "Network that grows every week",
    why1Text:
      "Your options increase day by day — new clinics and centers join constantly",
    why2Title: "Real discounts, no empty promises",
    why2Text: "Real savings up to 80% at clinics that carry our trusted seal",
    why3Title: "100% digital",
    why3Text:
      "Instant activation, instant use — everything on your phone, no paperwork",
    why4Title: "299 SAR for everything",
    why4Text:
      "One transparent price for a full year — no hidden fees, no surprises",
    finalCtaTitle: "How much will you save this year?",
    finalCtaSub:
      "Get your membership today with code GLOW1 and enjoy 10% off your first subscription.",
    finalCtaBtn: "Start saving now",
    finalCtaTrust: "✓ Your membership ready in one minute ✓ No commitments",
    networkPageTitle: "Our network grows every week — and so do your options",
    networkPageDesc:
      "One membership opens the door to clinics from beauty to dental, laser and skin, and health centers that never stop.",
    networkSpec1:
      "Beauty & skin clinics — Feel the difference from the first session",
    networkSpec2: "Dental clinics — Your smile deserves the best",
    networkSpec3: "Laser clinics — Lasting results at affordable prices",
    networkSpec4: "Dermatology clinics — Your real care starts here",
    networkSpec5: "Hair clinics — Because your hair is your crown",
    networkSpec6: "Eye clinics — Clear vision at a fair price",
    networkSpec7: "Nutrition clinics — Beauty starts from within",
    networkSpec8: "Sports clubs — Your fitness is part of your beauty",
    networkSpec9: "Beauty salons — Everything you need, with value and price",
    networkSpec10: "Spa centers — Moments of rest you deserve",
    membershipsPageTitle: "Which one suits you? — A membership for every life",
    membershipsPageDesc:
      "Five memberships, exclusive network, unlimited savings",
    Features: "our Features",
    featuresSub: "Save money on trusted services and providers",
    featuresSub1: "Medical Network",
    featuresSub2: "Membership Types",
    featuresSub3: "Profile",
    featuresSub4: "Glow Card Club",
    featuresSub5: "Save on doctor visits",
    featuresSub6:
      "Access to substantial savings on a wide range of medical services and treatments, helping you manage your healthcare costs effectively.",
    featuresSub7: "Pay less for doctor visits",
    featuresSub8:
      "Receive discounts on doctor consultations, making it more affordable to seek medical advice and routine check-ups.",
    featuresSub9: "No coupons needed",
    featuresSub10:
      "Enjoy the convenience of using your membership without the hassle of coupons or ongoing fees.",
    cards: "Our Memberships",
    city: "Glow Card Network",
    cityBtn: "<-- View More",
    Discount: "Discount",
    price: "Price",
    reqBtn: "Subscribe Now",
    howItWorks: "How It Works",
    desc: "Description",
    RequestCard: "Subscribe",
    RequestCardSub:
      "One membership, easy to use – choose the membership that suits your needs now!",
    cardInfo: "Membership Details",
    invitationLink: "Invitation Link",
    invitationLinkP:
      "Share this link with your friend to get two free months if they subscribe with us",
    logout: "log Out",
    cards: "Our Memberships",
    offers: "Our Offers",
    aboutus: "About us",
    aboutcompany: "Company Info",
    join: "Add Your Company",
    joinPageTitle: "Glow Card + your clinic = new clients every week",
    joinPageDesc:
      "Join the fastest-growing network in Saudi Arabia for beauty and wellness clinics — no joining fees, no financial commitments.",
    joinBenefit1:
      "Partner clinics in our network achieved this growth — 25–35% growth in new client numbers",
    joinBenefit2:
      "Free visibility in our marketing campaigns — your logo and offer reach thousands of subscribers monthly",
    joinBenefit3:
      "Reports and precise analytics — know where your new clients come from and how they behave",
    joinBenefit4:
      "Zero risk — no joining fees, no mandatory contracts, join with peace of mind",
    joinStepsHeading: "Steps to join",
    joinStep1:
      "Contact us — via website, phone or email — we respond within 24 hours",
    joinStep2:
      "Register your clinic — enter your business details in minutes, no complicated paperwork",
    joinStep3:
      "Set your offers — choose the services and discounts you provide",
    joinStep4:
      "Your clinic appears on the Glow Card network right away — approval to start immediately",
    joinCtaBtn: "Add your clinic now — free",
    new: "Newest Added",
    navNew: "Join recently",
    banner1:
      "Download the Glow Card app on Android & iOS — faster access to your discounts and clinics.",
    banner1GooglePlay: "Google Play",
    banner1AppStore: "App Store",
    banner2: "Your Local Health Discount Hub",
    banner3:
      "One membership, easy to use – choose the membership that suits your needs now!",
    banner4: "Discover",
    street: " المملكة العربية السعودية ، الرياض",
    aboutusSub1:
      "Glow Card is a digital membership that builds strong and lasting partnerships between businesses and clients.",
    aboutusSub2:
      "We provide discounts of up to 70% across a wide network of clinics, beauty salons, spa centers, wellness hubs, and health providers in the Kingdom.",
    aboutusSub3:
      "With Glow Card, you enjoy more than just savings—it’s your digital companion for beauty, wellness, and health.",
    aboutPageTitle: "Our story started with a simple question",
    aboutQuestion:
      "Why should care for beauty and health be a privilege for the few?",
    aboutStory:
      "Glow Card was born from this question — a simple idea that everyone deserves access to the best beauty and wellness clinics at a fair price.",
    aboutStory2:
      "Center by center, clinic by clinic — we're building the largest and most trusted network in the Kingdom today.",
    aboutMission:
      "Our mission: Build the largest specialized discount network that enables clinics and centers to grow and their customers to save.",
    aboutVision:
      "Our vision: To be the first trusted partner for every beauty and wellness clinic in the Kingdom of Saudi Arabia.",
    joinUs: "Join Us Now",
    success: "We Received Your Request Thanks",
    club: "Glow Club",
    glowClubPageTitle: "Welcome to Glow Club — where beauty meets excellence",
    glowClubPageDesc:
      "Glow Club is not just a membership — a community of conscious women who know the value of their time and money.",
    mdia: "Who Us ?",
    submedia:
      "Welcome to Glow Card where you can find all medical discount solutions.",
    other: "Other Details",
    career: "Careers",
    location: "Saudi Arabia, Riyadh",
    pdf: "Glow Card Portfolio",
    policyTitle: "Privacy Policy",
    policyDate: "Effective Date: 2025/09/01",
    policyIntro:
      "At Glow Card, we are committed to protecting the privacy and confidentiality of our customers’ data in compliance with the Personal Data Protection Law in the Kingdom of Saudi Arabia and related regulations. By using our services, you agree to this policy:",

    policySection1Title: "Information We Collect",
    policySection1_1:
      "Personal Identification Data: name, email, phone number.",
    policySection1_2:
      "Payment Data: Payments are processed through certified and secure providers; we do not store credit card information.",
    policySection1_3:
      "Usage Data: such as subscription history, your interactions with our services, and visits through the website or application.",

    policySection2Title: "How We Use the Data",
    policySection2_1: "Managing your account and providing technical support.",
    policySection2_2: "Sending alerts, updates, and service-related offers.",
    policySection2_3: "Improving service quality and digital experience.",
    policySection2_4: "Complying with legal and regulatory requirements.",

    policySection3Title: "Data Sharing",
    policySection3Intro:
      "Your data will not be shared with third parties except in the following cases:",
    policySection3_1:
      "With essential service providers (e.g., payment processors, technical support).",
    policySection3_2:
      "When there is a legal obligation or an official request from the competent authorities in Saudi Arabia.",
    policySection3_3:
      "Your data will never be sold or rented to any external entity under any circumstances.",

    policySection4Title: "Data Protection",
    policySection4_1:
      "We apply strict technical and organizational measures to protect your data from unauthorized access, use, alteration, or disclosure.",
    policySection4_2:
      "All data is handled according to the highest security standards and in compliance with local regulations.",

    policySection5Title: "User Rights",
    policySection5Intro:
      "According to Saudi regulations, you have the right to:",
    policySection5_1: "Request access to your personal data.",
    policySection5_2: "Request to modify or update your data.",
    policySection5_3:
      "Request the deletion of your data (unless retention is legally required).",
    policySection5_4:
      "Withdraw your consent to processing at any time (this may affect the ability to continue providing the service).",

    policySection6Title: "Data Retention",
    policySection6_1:
      "Your data will only be retained for as long as necessary to fulfill the purposes outlined above or as legally required.",

    policySection7Title: "Policy Changes",
    policySection7_1:
      "Glow Card reserves the right to update or amend the privacy policy at any time. You will be notified through the app or email if any material changes are made.",

    policySection8Title: "Contact Us",
    policySection8Intro:
      "For inquiries or to exercise your data-related rights, please contact us at:",
    termsTitle: "Terms and Conditions",

    termsSection1Title: "1. Permitted Use",
    termsSection1_1:
      "The official Glow Card website and application are designated solely for accessing its digital services. Any other use outside this scope is strictly prohibited.",

    termsSection2Title: "2. Subscriptions and Discounts",
    termsSection2_1:
      "A Glow Card subscription grants the subscriber access to discounts offered within our partner network (clinics, medical centers, salons, wellness hubs).",
    termsSection2_2:
      "The discount rate and type of service may vary depending on contractual agreements with partners.",

    termsSection3Title: "3. Modifications to the Medical/Beauty Network",
    termsSection3_1:
      "Glow Card reserves the right to add or remove any clinic or center from its network during the subscription period, due to contract expiration, non-renewal, or other operational/contractual reasons.",
    termsSection3_2:
      "Continuing a subscription does not guarantee that all listed centers will remain unchanged.",

    termsSection4Title: "4. Modification of Discounts",
    termsSection4_1:
      "Discount percentages may be increased or decreased by partners in line with contract renewals or modifications.",
    termsSection4_2:
      "Glow Card updates these changes periodically on its official platform, and reviewing them is considered part of the customer’s responsibility under these terms.",

    termsSection5Title: "5. Legal Use",
    termsSection5_1:
      "Any commercial, illegal, or defamatory use of Glow Card or its partners is strictly prohibited.",
    termsSection5_2:
      "Any attempt to misuse the subscription may lead to its cancellation without compensation.",

    termsSection6Title: "6. Intellectual Property",
    termsSection6_1:
      "All trademarks, logos, designs, content, and promotional materials are the exclusive property of Glow Card.",
    termsSection6_2:
      "Reproduction, use, or exploitation without prior written consent from Glow Card is strictly prohibited.",

    termsSection7Title: "7. Amendments to Terms and Conditions",
    termsSection7_1:
      "Glow Card reserves the right to amend these Terms and Conditions at any time.",
    termsSection7_2:
      "Continued use of the services after such amendments constitutes explicit acceptance of the updated terms.",

    termsSection8Title: "8. Disclaimer",
    termsSection8_1:
      "Glow Card is not responsible for the quality of medical or cosmetic services provided by partners, as its role is limited to offering a membership that connects subscribers with contracted centers.",
    termsSection8_2:
      "All medical and professional responsibilities rest with the center/clinic providing the service.",

    termsSection9Title: "9. Subscriptions and Payment",
    termsSection9_1: "The subscription fee is non-refundable once activated.",
    termsSection9_2:
      "The subscription is personal and non-transferable, non-saleable, and cannot be shared with others.",

    termsSection10Title: "10. Duration and Service Termination",
    termsSection10_1:
      "The subscription duration is determined according to the chosen package.",
    termsSection10_2:
      "Glow Card has the right to suspend or terminate a subscriber’s service if the terms are violated, without any financial liability.",
    returnTitle: "Return and Refund Policy",

    returnSection1Title: "1. No Refunds After Activation",
    returnSection1_1:
      "Subscribers are not entitled to refund or return the subscription fee once the digital membership has been activated or used in any Glow Card service.",

    returnSection2Title: "2. Grace Period Before Activation",
    returnSection2_1:
      "If the membership is not activated within 7 days from the date of purchase, the customer is entitled to request a full refund.",
    returnSection2_2:
      "The refund will be processed using the same payment method within a maximum of 14 business days.",

    returnSection3Title: "3. Technical Issues",
    returnSection3_1:
      "If a technical issue prevents the subscriber from using the membership or accessing services, they are entitled to:",
    returnSection3_2:
      "Request technical support to resolve the issue, or request a replacement membership.",
    returnSection3_3:
      "If the issue cannot be resolved, the customer is entitled to a full refund.",

    returnSection4Title: "4. Exceptions",
    returnSection4_1: "Refunds do not apply in cases resulting from:",
    returnSection4_2:
      "Incorrect data entered by the customer, unauthorized use, or violation of the terms and conditions.",
    returnSection4_3:
      "Issues caused by partner clinics or centers (service quality or appointments).",

    returnSection5Title: "5. Refund Request Procedure",
    returnSection5_1:
      "The customer must submit an official request via email to: support@glowcard.com.sa, clearly stating the reason and attaching purchase details.",
    returnSection5_2:
      "Glow Card reserves the right to verify the validity of the request before approving a refund or replacement.",
    salesTitle: "Sales and Usage Policy",

    salesSection1Title: "1. Nature of the Memberships",
    salesSection1_1:
      "All Glow Card memberships are 100% digital and are issued electronically via the website or application after successful payment.",
    salesSection1_2: "No physical cards are shipped or delivered.",

    salesSection2Title: "2. Subscription Start and Activation",
    salesSection2_1:
      "The subscription begins immediately upon payment confirmation and issuance of the digital membership to the user.",
    salesSection2_2:
      "The subscription is personal, linked to the user’s account, and cannot be transferred or shared with others.",

    salesSection3Title: "3. Prices and Fees",
    salesSection3_1:
      "All published prices include VAT applicable in the Kingdom of Saudi Arabia in accordance with the law.",
    salesSection3_2:
      "Glow Card reserves the right to modify subscription or package prices at any time, with official announcements made before application.",

    salesSection4Title: "4. Permitted Usage",
    salesSection4_1:
      "Glow Card services are limited to enjoying discounts and offers within its contracted medical, cosmetic, and wellness network.",
    salesSection4_2:
      "Reselling the membership or exploiting it for commercial or illegal purposes is strictly prohibited.",
    salesSection4_3:
      "Any violation of these terms will result in immediate subscription cancellation without refund.",

    salesSection5Title: "5. No Refund or Cancellation",
    salesSection5_1:
      "Subscription fees are non-refundable once payment is completed and the digital membership is activated, regardless of usage.",
    salesSection5_2:
      "Subscribers are not entitled to refunds in case of network changes, center removals, or discount modifications.",

    salesSection6Title: "6. Network and Service Changes",
    salesSection6_1:
      "Glow Card reserves the right to add or remove any center or modify discounts within its network during the subscription period, subject to partner contract renewals or adjustments.",
    salesSection6_2:
      "Continued use of the membership is considered implicit acceptance of these changes.",

    salesSection7Title: "7. Liability",
    salesSection7_1:
      "Glow Card bears no responsibility for the quality of services provided by partner centers or clinics. Its role is limited to offering the membership as a tool to connect subscribers with partners.",
    userName: "User Name",
    email: "Email",
    city: "City",
    choose: "Choose City",
    message: "Your Message",
    special: "Specializations",
    open_pdf_btn: "Open Portfolio",
    activation: "Membership Activation",
    contact: "Contact Us",
    contactPageTitle: "We're here — talk to us",
    contactPageSub:
      "Whether you're a subscriber with a question or want to join — a clinic or company looking for corporate membership — our team responds within hours.",
    contactLocation: "Riyadh — Kingdom of Saudi Arabia",
    contactPhone: "966+ 542220888",
    contactEmail: "info@glowcard.com.sa",
    contactWhatsAppBtn: "Talk to us on WhatsApp now",
    subContact:
      "We are here for you! Feel free to reach out to our friendly team anytime — we are happy to guide you, answer your questions, and help you start saving on healthcare costs today.",
    subContact2: "Ready to start using your membership?",
    subContact3:
      "Let us turn your vision into reality. Contact us today to discuss how we can help you innovate and grow.",
    contactways: "Contact Methods",
    howItWorks2:
      "Enjoy easy and quick access to your digital membership through the app by following simple steps:",
    howItWorks3: "- Subscribe Now – Sign up to get your digital membership",
    howItWorks4: "- Activate your membership – in easy and quick steps",
    howItWorks5: "- Start saving – Enjoy exclusive discounts and offers",
    howItWorks6: "- Renew your subscription – to continue enjoying discounts",
    howItWorks7:
      "- Share with friends – Get additional benefits when you invite your friends to subscribe",
    RequestCompanyCard: "Request Company Membership",
    companyName: "Company Name",
    CompanyEmployees: "Number of Company Employees",
    AllFieldsRequired: "All fields are required",
    appPageTitle: "Subscribe",
    appPageSubtitle:
      "One membership, easy to use – choose the membership that suits your needs now!",
    appFormStepFather: "Main applicant",
    appFormStepMother: "Spouse application",
    appFormStepChild1: "Child 1 application",
    appFormStepChild2: "Child 2 application",
    appLabelName: "Name",
    appPhName: "Full name",
    appLabelPhone: "Phone",
    appPhPhone: "Phone number",
    appLabelEmail: "Email",
    appPhEmail: "you@example.com",
    appLabelGender: "Gender",
    appGenderPlaceholder: "Select gender",
    appGenderMale: "Male",
    appGenderFemale: "Female",
    appLabelAddress: "Address",
    appPhAddress: "Address",
    appLabelNationalId: "National ID",
    appPhNationalId: "National ID",
    appLabelDob: "Date of birth",
    appLabelNationality: "Nationality",
    appPhNationality: "Nationality",
    appBtnBack: "Back",
    appBtnNext: "Next",
    appBtnActivate: "Activate",
    appBtnLoading: "Loading...",
    appAlertAllRequired: "All fields are required.",
    appModalThanks: "Thank you for subscribing!",
    appModalDesc: "Go to your membership details to continue.",
    appModalLink: "Membership details",
    appModalClose: "Close",
  },
  ar: {
    Login: "تسجيل دخول",
    phone: "رقم الهاتف",
    googleAuth: "سجل دخول بواسطة جوجل",
    loginTitle: "استمتع براحة استخدام عضويتك",
    loginSubTitle: "إطلاق ميزات جديدة",
    loginSubTitle2:
      "احصل على أسعار مخفضة في أفضل المراكز الطبية دون دفع السعر الكامل.",
    home: "الرئيسية",
    network: "العيادات والمراكز",
    contactUs: "تواصل معنا",
    hero: "جمالك يستحق عضوية المشاهير",
    heroSub: "عضوية المشاهير في الجمال والصحة",
    heroBullet1: "خصومات حصرية تصل إلى 80% ✦ شبكة عيادات معتمدة ومتنامية",
    heroBullet2: "عضوية رقمية كاملة بـ 299 ريال / السنة",
    heroBullet3: "فعّالة فوراً ✓ بدون شروط خفية",
    heroBtn: "احصلي على عضويتك",
    heroTrust: "✓ رقمية بالكامل ✓ فعّالة فوراً ✓ بدون شروط خفية",
    howSectionTitle: "كيف تعمل جلو كارد؟",
    howStep1:
      "اختاري عضويتك — كلاسيك، بريميوم، أزواج، عائلات، شركات — أو اختاري ما يناسب حياتك",
    howStep2:
      "فعّلي عضويتك — ادفعي 299 ريال، تنشط عضويتك في نفس اللحظة وتبدأ توفيرك فوراً",
    howStep3:
      "اختاري عيادتك — تصفّحي شبكتنا المتنامية من عيادات الجمال والصحة المعتمدة في مدينتك",
    howStep4:
      "اعرضي عضويتك الرقمية في العيادة — وفّري واحصلي على خصمك فوراً، بدون أوراق أو كوبونات",
    whySectionTitle: "لماذا جلو كارد؟",
    why1Title: "شبكة تتوسع كل أسبوع",
    why1Text: "خياراتك تزيد يوم بعد يوم — عيادات ومراكز جديدة تنضم باستمرار",
    why2Title: "خصومات حقيقية، لا وعود",
    why2Text: "نسبة توفير حقيقية تصل إلى 80% في عيادات تحمل علامتنا المعتمدة",
    why3Title: "بالكامل رقمية",
    why3Text: "تفعيل فوري، استخدام فوري — كل شيء في هاتفك بدون أوراق",
    why4Title: "299 ريال — شيء واحد",
    why4Text: "سعر واحد شفاف وكامل لسنة — لا رسوم خفية، لا مفاجآت",
    finalCtaTitle: "كم ستوفّرين هذا العام؟",
    finalCtaSub:
      "احصلي على عضويتك اليوم بكود GLOW1 واستمتعي بخصم 10% على أول اشتراك.",
    finalCtaBtn: "ابدئي التوفير الآن",
    finalCtaTrust: "✓ عضويتك جاهزة خلال دقيقة واحدة ✓ بدون أي التزامات",
    networkPageTitle: "شبكتنا تكبر كل أسبوع — معها خياراتك",
    networkPageDesc:
      "عضوية واحدة تفتح لك أبواب عيادات من التجميل إلى الأسنان والليزر والجلدية ومراكز الصحّة — لا تنتهي.",
    networkSpec1: "✨ عيادات التجميل والبشرة — أشعري بالفرق من أول جلسة",
    networkSpec2: "🦷 عيادات الأسنان — ابتسامتك تستحق الأفضل",
    networkSpec3: "⚡ عيادات الليزر — نتائج دائمة بأسعار ممكنة",
    networkSpec4: "🌿 عيادات الجلدية — هنا تبدأ عنايتك الحقيقية",
    networkSpec5: "💇 عيادات الشعر — لأن شعرك تاجك",
    networkSpec6: "👁️ عيادات العيون — وضوح الرؤية بثمن يُقدّر",
    networkSpec7: "🥗 عيادات التغذية — الجمال يبدأ من الداخل",
    networkSpec8: "💪 نوادي رياضية — لياقتك جزء من جمالك",
    networkSpec9: "💄 صالونات التجميل — كل ما تحتاجينه بميزة وسعر",
    networkSpec10: "🛁 مراكز السبا — لحظات راحة تستحقينها",
    membershipsPageTitle: "أيّتها تناسبك؟ — حياة لكل عضوية",
    membershipsPageDesc: "خمس عضويات، شبكة حصرية، توفير لا محدود",
    Features: "مميزاتنا",
    featuresSub: "وفّري على خدماتك الجمالية والمزودين الموثوق بهم",
    featuresSub1: "العيادات والمراكز",
    featuresSub2: "أنواع العضويات",
    featuresSub3: "البروفايل",
    featuresSub4: "نادي جلو",
    featuresSub5: "وفّري على زياراتك الجمالية والصحية",
    featuresSub6:
      "احصل على خصومات كبيرة على مجموعة واسعة من الخدمات والعلاجات الطبية، مما يساعدك على إدارة تكاليف الرعاية الصحية الخاصة بك بشكل فعال.",
    featuresSub7: "احصلي على خصومات في عيادات الجمال والصحة",
    featuresSub8:
      "احصل على خصومات على استشارات الأطباء، مما يجعل الحصول على المشورة الطبية والفحوصات الروتينية أقل تكلفة.",
    featuresSub9: "عضويتك الرقمية تفعل كل شيء — لا متاعب، لا كوبونات!",
    featuresSub10:
      "استمتع بسهولة استخدام عضويتك دون متاعب القسائم أو الرسوم المستمرة.",
    cards: "عضوياتنا المميزة",
    city: "شبكة جلو كارد",
    cityBtn: "<-- المزيد",
    Discount: "خصم",
    price: "سعر",
    reqBtn: "اشترك الآن",
    howItWorks: "طريقة الاستخدام",
    desc: "وصف العضوية",
    RequestCard: "اشترك الآن",
    RequestCardSub:
      "عضوية واحدة، سهل الاستخدام – اختر العضوية التي تناسب احتياجاتك الآن!",
    cardInfo: "تفاصيل العضوية",
    invitationLink: "رابط الدعوة",
    invitationLinkP:
      "شارك هذا الرابط مع صديقك للحصول على شهرين مجانيين في حال اشتراكه معنا",
    logout: "تسجيل الخروج",
    cards: "الاشتراكات",
    offers: "العروض",
    aboutus: "عن جلو كارد",
    aboutcompany: "بيانات الشركة",
    join: "أضف شركتك",
    joinPageTitle: "كارد جلو + عيادتك = عملاء جدد كل أسبوع",
    joinPageDesc:
      "انضم إلى أسرع شبكة نمواً في السعودية لعيادات الجمال والصحة — انضمام بدون رسوم، بدون التزامات مالية.",
    joinBenefit1:
      "عيادات شريكة فعلية في شبكتنا حققت هذا النمو — نمو 25–35% في عدد العملاء الجدد",
    joinBenefit2:
      "ظهور مجاني في حملاتنا التسويقية — شعارك وعرضك يصل لآلاف المشتركين شهرياً",
    joinBenefit3:
      "تقارير وتحليلات دقيقة — اعرف من أين يأتي عملاؤك الجدد وكيف يتصرفون",
    joinBenefit4:
      "صفر مخاطر — لا رسوم إنضمام، لا عقود إلزامية، إنضم وأنت مرتاح",
    joinStepsHeading: "خطوات الانضمام",
    joinStep1:
      "تواصلي معنا — عبر الموقع أو الهاتف أو البريد — نرد خلال 24 ساعة",
    joinStep2:
      "سجّل عيادتك — أدخلي بيانات نشاطك التجاري في دقائق، بدون أوراق معقدة",
    joinStep3: "حدّدي عروضك — من أن تختاري الخدمات والخصومات التي تقدمينها",
    joinStep4: "فوراً — عيادتك تظهر في شبكة جلو كارد، الموافقة فور البدء",
    joinCtaBtn: "أضيفي عيادتك الآن — مجاناً",
    new: "إنضمّ حديثاً",
    navNew: "إنضمّ حديثاً",
    banner1:
      "حمّل تطبيق جلو كارد على Android و iOS — وصول أسرع لخصوماتك وعياداتك.",
    banner1GooglePlay: "Google Play",
    banner1AppStore: "App Store",
    banner2:
      "مركز الخصم الصحي المحلي الخاص بك — تفتح لك عالم من الخصومات في أكبر شبكة عيادات جمالية وصحية في المملكة",
    banner3:
      "عضوية واحدة، سهل الاستخدام – اختر العضوية التي تناسب احتياجاتك الآن!",
    banner4: "اكتشف",
    street: " المملكة العربية السعودية ، الرياض",
    aboutusSub1:
      "جلو كارد هي عضوية رقمية تهدف إلى بناء شراكات ناجحة بين مختلف المؤسسات والعملاء",
    aboutusSub2:
      "‎نوفّر خصومات تصل حتى %70 ضمن شبكة واسعة من العيادات، صالونات التجميل، مراكز السبا والعناية بالصحة والرفاهية، والعديد من مقدّمي الخدمات في المملكة.",
    aboutusSub3:
      "مع جلو كارد، تحصل على أكثر من مجرد خصومات، فهي رفيقك الرقمي للجمال، الصحة، والرفاهية",
    aboutPageTitle: "قصتنا بدأت من سؤال بسيط",
    aboutQuestion: "لماذا يجب أن تكون العناية بالجمال والصحة امتيازاً للقلة؟",
    aboutStory:
      "جلو كارد وُلدت من هذا السؤال — فكرة بسيطة تؤمن بأن كل شخص يستحق الوصول إلى أفضل عيادات الجمال والصحة بسعر عادل.",
    aboutStory2:
      "مركز مركز، عيادة عيادة — نبني اليوم الأكبر والأكثر ثقة في المملكة.",
    aboutMission:
      "رسالتنا: نبني أكبر شبكة خصومات متخصصة تُمكّن العيادات والمراكز من النمو وعملاءها من التوفير.",
    aboutVision:
      "رؤيتنا: أن نكون الشريك الموثوق الأول لكل عيادة جمال وصحة في المملكة العربية السعودية.",
    joinUs: "انضم الينا الآن",
    success: "تم إرسال الطلب بنجاح",
    club: "نادي جلو",
    glowClubPageTitle: "مرحباً بك في نادي جلو — حيث يلتقي الجمال بالامتياز",
    glowClubPageDesc:
      "نادي جلو ليس مجرد عضوية — مجتمع نساء واعيات يعرفن قيمة وقتهن ومالهن.",
    mdia: "من نحن؟",
    submedia: "مرحبا بكم في جلو كارد حيث يوجد جميع حلول الخصومات الطبية",
    other: "تفاصيل أخرى",
    career: "التوظيف",
    location: "المملكة العربية السعودية ، الرياض",
    pdf: "الملف لتعريفي",
    policyTitle: "سياسة الخصوصية",
    policyDate: "تاريخ السريان: 2025/09/01",
    policyIntro:
      "نحن في Glow Card ملتزمون بحماية خصوصية وسرية بيانات عملائنا بما يتوافق مع نظام حماية البيانات الشخصية في المملكة العربية السعودية واللوائح ذات الصلة. باستخدامك لخدماتنا، فإنك توافق على هذه السياسة:",

    policySection1Title: "المعلومات التي نجمعها",
    policySection1_1:
      "بيانات التعريف الشخصية: الاسم، البريد الإلكتروني، رقم الجوال.",
    policySection1_2:
      "بيانات الدفع: تتم معالجة المدفوعات عبر مزودي دفع معتمدين وآمنين، ولا نقوم بتخزين بيانات البطاقات البنكية.",
    policySection1_3:
      "بيانات الاستخدام: مثل سجل الاشتراكات، تفاعلك مع الخدمات، وزياراتك عبر الموقع أو التطبيق.",

    policySection2Title: "كيفية استخدام البيانات",
    policySection2_1: "إدارة حسابك وتقديم الدعم الفني.",
    policySection2_2: "إرسال التنبيهات، التحديثات، والعروض الخاصة بالخدمة.",
    policySection2_3: "تحسين جودة الخدمات والتجربة الرقمية.",
    policySection2_4: "الالتزام بالمتطلبات القانونية والتنظيمية.",

    policySection3Title: "مشاركة البيانات",
    policySection3Intro:
      "لا تتم مشاركة بياناتك مع أي طرف ثالث إلا في الحالات التالية:",
    policySection3_1:
      "مع مزودي الخدمة الضروريين (مثل مزودي الدفع، الدعم الفني).",
    policySection3_2:
      "عند وجود التزام قانوني أو طلب رسمي من الجهات المختصة في المملكة العربية السعودية.",
    policySection3_3: "لن يتم بيع أو تأجير بياناتك لأي جهة خارجية تحت أي ظرف.",

    policySection4Title: "حماية البيانات",
    policySection4_1:
      "نتخذ إجراءات تقنية وتنظيمية صارمة لحماية بياناتك من الوصول غير المصرح به، أو الاستخدام، أو التغيير، أو الإفشاء.",
    policySection4_2:
      "يتم التعامل مع جميع البيانات وفق أعلى معايير الأمان وبما يتوافق مع اللوائح المحلية.",

    policySection5Title: "حقوق المستخدم",
    policySection5Intro: "وفقًا للأنظمة السعودية، يحق لك:",
    policySection5_1: "طلب الاطلاع على بياناتك الشخصية.",
    policySection5_2: "طلب تعديل أو تحديث بياناتك.",
    policySection5_3:
      "طلب حذف بياناتك (ما لم يكن الاحتفاظ بها مطلوبًا قانونيًا).",
    policySection5_4:
      "سحب موافقتك على المعالجة في أي وقت (قد يؤثر ذلك على إمكانية استمرار تقديم الخدمة).",

    policySection6Title: "الاحتفاظ بالبيانات",
    policySection6_1:
      "يتم الاحتفاظ ببياناتك فقط للمدة اللازمة لتحقيق الأغراض المذكورة أعلاه أو حسب ما يقتضيه النظام.",

    policySection7Title: "التعديلات على السياسة",
    policySection7_1:
      "تحتفظ Glow Card بحقها في تحديث أو تعديل سياسة الخصوصية في أي وقت. وسيتم إشعارك عبر التطبيق أو البريد الإلكتروني عند إجراء تغييرات جوهرية.",

    policySection8Title: "التواصل معنا",
    policySection8Intro:
      "للاستفسارات أو ممارسة حقوقك المتعلقة ببياناتك، يرجى التواصل معنا عبر:",
    termsTitle: "الشروط والأحكام",

    termsSection1Title: "1. الاستخدام المخصص",
    termsSection1_1:
      "يُخصص الموقع الإلكتروني والتطبيق الرسمي لـ Glow Card للوصول إلى خدماتها الرقمية فقط، ويُحظر أي استخدام آخر خارج هذا الإطار.",

    termsSection2Title: "2. الاشتراكات والخصومات",
    termsSection2_1:
      "الاشتراك في Glow Card يمنح المشترك إمكانية الاستفادة من الخصومات المقدمة ضمن شبكة شركائنا (العيادات، المراكز الطبية، الصالونات، المراكز الصحية).",
    termsSection2_2:
      "نسبة الخصم ونوع الخدمة يختلفان حسب ظروف التعاقد مع الشركاء.",

    termsSection3Title: "3. تعديل الشبكة الطبية/التجميلية",
    termsSection3_1:
      "تحتفظ Glow Card بالحق في إضافة أو حذف أي عيادة أو مركز من شبكتها خلال فترة الاشتراك، وذلك في حال انتهاء التعاقد أو عدم تجديده أو لأي أسباب تشغيلية أو تعاقدية أخرى.",
    termsSection3_2: "استمرار الاشتراك لا يضمن بقاء جميع المراكز دون تغيير.",

    termsSection4Title: "4. تعديل نسب الخصومات",
    termsSection4_1:
      "قد يتم تعديل نسبة الخصم (رفعها أو خفضها) من قِبل الشركاء وفقًا لتجديد العقود أو تعديلها.",
    termsSection4_2:
      "تُحدّث Glow Card هذه التغييرات بشكل دوري في منصتها الرسمية، ويُعتبر اطلاع العميل عليها جزءًا من التزامه بالشروط.",

    termsSection5Title: "5. الاستخدام القانوني",
    termsSection5_1:
      "يحظر تمامًا أي استخدام تجاري، غير قانوني، أو يسيء لسمعة Glow Card أو شركائها.",
    termsSection5_2:
      "أي محاولة إساءة استخدام الاشتراك قد تؤدي إلى إلغائه دون تعويض.",

    termsSection6Title: "6. الملكية الفكرية",
    termsSection6_1:
      "جميع العلامات التجارية، الشعارات، التصاميم، المحتوى، والمواد الدعائية مملوكة حصريًا لـ Glow Card.",
    termsSection6_2:
      "يُحظر إعادة نسخها أو استخدامها أو استغلالها دون إذن خطي مسبق من Glow Card.",

    termsSection7Title: "7. تعديل الشروط والأحكام",
    termsSection7_1:
      "تحتفظ Glow Card بحقها في تعديل هذه الشروط والأحكام في أي وقت.",
    termsSection7_2:
      "يُعد استمرار استخدام المشترك للخدمات بعد التعديلات موافقة صريحة منه على الشروط الجديدة.",

    termsSection8Title: "8. إخلاء المسؤولية",
    termsSection8_1:
      "Glow Card ليست مسؤولة عن جودة الخدمات الطبية أو التجميلية المقدمة من الشركاء، حيث أن دورها يقتصر على توفير عضوية تربط المشتركين بالمراكز المتعاقدة.",
    termsSection8_2:
      "جميع المسؤوليات الطبية والمهنية تقع على عاتق المركز/العيادة المقدمة للخدمة.",

    termsSection9Title: "9. الاشتراكات والدفع",
    termsSection9_1: "قيمة الاشتراك غير قابلة للاسترداد بعد تفعيله.",
    termsSection9_2:
      "الاشتراك شخصي وغير قابل للتحويل أو البيع أو المشاركة مع الغير.",

    termsSection10Title: "10. المدة وإنهاء الخدمة",
    termsSection10_1: "مدة الاشتراك تُحدد وفق الباقة المختارة.",
    termsSection10_2:
      "لـ Glow Card الحق في إيقاف أو إنهاء الخدمة للمشترك في حال مخالفة الشروط دون التزام بأي تعويض مالي.",
    returnTitle: "سياسة الإرجاع والاسترداد",

    returnSection1Title: "1. عدم الاسترجاع بعد التفعيل",
    returnSection1_1:
      "لا يحق للمشترك استرجاع أو استرداد قيمة الاشتراك بعد تفعيل العضوية الرقمية أو استخدامها في أي من خدمات Glow Card.",

    returnSection2Title: "2. فترة السماح قبل التفعيل",
    returnSection2_1:
      "إذا لم يتم تفعيل العضوية خلال 7 أيام من تاريخ الشراء، يحق للعميل طلب استرداد كامل المبلغ المدفوع.",
    returnSection2_2:
      "تتم عملية الاسترداد عبر نفس وسيلة الدفع المستخدمة، خلال مدة لا تتجاوز 14 يوم عمل.",

    returnSection3Title: "3. المشكلات التقنية",
    returnSection3_1:
      "في حال وجود مشكلة تقنية تمنع المشترك من الاستفادة من العضوية أو استخدام الخدمات بشكل كامل، يحق له:",
    returnSection3_2:
      "طلب حل المشكلة التقنية من فريق الدعم، أو استبدال العضوية بعضوية جديدة.",
    returnSection3_3: "أو استرداد المبلغ كاملاً في حال تعذر الحل.",

    returnSection4Title: "4. الاستثناءات",
    returnSection4_1: "لا يشمل الاسترداد الحالات الناتجة عن:",
    returnSection4_2:
      "إدخال بيانات خاطئة من قبل العميل، استخدام غير مصرح به أو مخالف للشروط والأحكام.",
    returnSection4_3:
      "مشاكل ناتجة عن المراكز أو العيادات الشريكة (جودة الخدمة أو مواعيدها).",

    returnSection5Title: "5. إجراءات طلب الاسترداد",
    returnSection5_1:
      "على العميل التقدم بطلب رسمي عبر البريد الإلكتروني المخصص: support@glowcard.com.sa مع توضيح سبب الطلب وإرفاق بيانات الشراء.",
    returnSection5_2:
      "تحتفظ Glow Card بحق التحقق من صحة الطلب قبل الموافقة على الاسترداد أو الاستبدال.",
    salesTitle: "سياسة البيع والاستخدام",

    salesSection1Title: "1. طبيعة العضويات",
    salesSection1_1:
      "جميع عضويات Glow Card رقمية 100%، ويتم إصدارها إلكترونيًا عبر الموقع أو التطبيق بعد إتمام الدفع بنجاح.",
    salesSection1_2: "لا يتم شحن أو تسليم أي بطاقات مادية.",

    salesSection2Title: "2. بدء الاشتراك وتفعيله",
    salesSection2_1:
      "يبدأ الاشتراك فور تأكيد عملية الدفع وإصدار العضوية الرقمية للمستخدم.",
    salesSection2_2:
      "الاشتراك شخصي ومقترن بحساب المستخدم ولا يجوز مشاركته أو نقله للغير.",

    salesSection3Title: "3. الأسعار والرسوم",
    salesSection3_1:
      "جميع الأسعار المعلنة تشمل ضريبة القيمة المضافة المطبقة في المملكة العربية السعودية وفق النظام.",
    salesSection3_2:
      "يحق لـ Glow Card تعديل أسعار الاشتراكات أو الباقات في أي وقت، مع الإعلان عنها بشكل رسمي قبل التطبيق.",

    salesSection4Title: "4. الاستخدام المسموح",
    salesSection4_1:
      "تقتصر خدمات Glow Card على الاستفادة من الخصومات والعروض داخل شبكة المراكز الطبية والتجميلية والصحية المتعاقدة مع الشركة.",
    salesSection4_2:
      "يحظر إعادة بيع العضوية أو استغلالها لأغراض تجارية أو غير قانونية.",
    salesSection4_3:
      "أي استخدام مخالف للشروط يؤدي إلى إلغاء الاشتراك فورًا دون استرداد أي مبالغ.",

    salesSection5Title: "5. عدم الاسترداد أو الإلغاء",
    salesSection5_1:
      "رسوم الاشتراك غير قابلة للاسترداد بعد إتمام عملية الدفع وتفعيل العضوية الرقمية، سواء تم استخدام العضوية أو لم يتم استخدامها.",
    salesSection5_2:
      "لا يحق للمشترك المطالبة باسترداد قيمة الاشتراك عند تغيير أو إلغاء أحد المراكز أو تعديل نسب الخصومات.",

    salesSection6Title: "6. تغييرات الشبكة والخدمات",
    salesSection6_1:
      "تحتفظ Glow Card بحقها في إضافة أو حذف أي مركز أو تعديل نسب الخصومات ضمن شبكتها خلال فترة الاشتراك، تبعًا لتجديد أو تعديل العقود مع الشركاء.",
    salesSection6_2:
      "استمرار استخدام العضوية يُعد موافقة ضمنية من المشترك على هذه التغييرات.",

    salesSection7Title: "7. المسؤولية",
    salesSection7_1:
      "Glow Card لا تتحمل أي التزام أو مسؤولية عن جودة الخدمات المقدمة من المراكز أو العيادات الشريكة، حيث يقتصر دورها على توفير العضوية كوسيلة خصم وربط المشترك بالشركاء.",
    userName: "اسم المستخدم",
    email: "البريد الإلكتروني",
    city: "المدينة",
    choose: "اختر المدينة",
    message: "الرسالة",
    special: "التخصصات",
    open_pdf_btn: "فتح الملف التعريفي",
    activation: "تفعيل العضويات",
    contact: "تواصل معنا",
    contactPageTitle: "نحن هنا — تحدّثي معنا",
    contactPageSub:
      "سواء كنت مشتركة أو لديها سؤال أو تريد الانضمام — عيادة أو شركة تبحث عن عضوية مؤسسية، فريقنا يرد خلال ساعات.",
    contactLocation: "الرياض — المملكة العربية السعودية",
    contactPhone: "966+ 542220888",
    contactEmail: "info@glowcard.com.sa",
    contactWhatsAppBtn: "تحدّثي معنا على واتساب الآن",
    subContact:
      "نحن هنا من أجلك! لا تتردد في التواصل مع فريقنا الودود في أي وقت — يسعدنا إرشادك، والإجابة على أسئلتك، ومساعدتك في البدء بتوفير تكاليف الرعاية الصحية اليوم.",
    subContact2: "جاهز لاستخدام عضويتك؟",
    subContact3:
      "دعنا نحول رؤيتك إلى واقع. تواصل معنا اليوم لنتحدث عن كيفية مساعدتك على الابتكار والنمو.",
    contactways: "طرق التواصل",
    howItWorks2:
      "استمتع بسهولة وسرعة الوصول إلى عضويتك الرقمية عبر التطبيق، باتباع خطوات بسيطة:",
    howItWorks3: "- اشترك الآن – قم بالتسجيل للحصول على عضويتك الرقمية",
    howItWorks4: "- قم بتفعيل عضويتك – في خطوات سهلة وسريعة",
    howItWorks5: "- ابدأ التوفير – استمتع بالخصومات والعروض الحصرية",
    howItWorks6: "- جدّدي عضويتك – للحفاظ على استمرارية الاستفادة من الخصومات",
    howItWorks7:
      "- شارك مع الأصدقاء – احصل على مزايا إضافية عند دعوة أصدقائك للاشتراك",
    RequestCompanyCard: "طلب عضوية شركة",
    companyName: "اسم الشركة",
    CompanyEmployees: "عدد موظفي الشركة",
    AllFieldsRequired: "جميع الحقول مطلوبة",
    appPageTitle: "اشتراك وتفعيل",
    appPageSubtitle:
      "عضوية واحدة، سهل الاستخدام – اختر العضوية التي تناسب احتياجاتك الآن!",
    appFormStepFather: "البيانات الأساسية",
    appFormStepMother: "بيانات الزوج / الزوجة",
    appFormStepChild1: "بيانات الطفل الأول",
    appFormStepChild2: "بيانات الطفل الثاني",
    appLabelName: "الاسم",
    appPhName: "الاسم الكامل",
    appLabelPhone: "الجوال",
    appPhPhone: "رقم الجوال",
    appLabelEmail: "البريد الإلكتروني",
    appPhEmail: "you@example.com",
    appLabelGender: "الجنس",
    appGenderPlaceholder: "اختر الجنس",
    appGenderMale: "ذكر",
    appGenderFemale: "أنثى",
    appLabelAddress: "العنوان",
    appPhAddress: "العنوان",
    appLabelNationalId: "رقم الهوية",
    appPhNationalId: "رقم الهوية الوطنية",
    appLabelDob: "تاريخ الميلاد",
    appLabelNationality: "الجنسية",
    appPhNationality: "الجنسية",
    appBtnBack: "رجوع",
    appBtnNext: "التالي",
    appBtnActivate: "تفعيل",
    appBtnLoading: "جاري التحميل...",
    appAlertAllRequired: "كل الحقول مطلوبة.",
    appModalThanks: "شكراً لاشتراكك معنا!",
    appModalDesc: "يرجى التوجه إلى تفاصيل العضوية لمتابعة العملية.",
    appModalLink: "تفاصيل العضوية",
    appModalClose: "إغلاق",
  },
};
